...
new #[Layout('layouts.app')] class extends Component {
    ...
    public $new_ep_thumbnail; // Opsional
    public string $creator_note = '';
    public bool $allow_comments = true;
    public string $published_at = '';

    // FAKTA: Arsitektur Unified State (Sama Persis dengan Create)
    ...
    {
        if ($chapter->comic->author_id !== Auth::id()) {
            abort(403, 'Akses Ditolak. Ini bukan karya Anda.');
        }
        if ($chapter->status === 'approved') {
            session()->flash('error', 'Episode yang sudah diterbitkan tidak dapat direvisi secara langsung.');
            return redirect()->route('author.episodes.index', $chapter->comic_id);
        }

        $this->chapter = $chapter;
        $this->comic = $chapter->comic;
        $this->ep_title = $chapter->title;
        $this->creator_note = $chapter->creator_note ?? '';
        $this->allow_comments = $chapter->allow_comments;
        $this->published_at = $chapter->published_at ? Carbon::parse($chapter->published_at)->format('Y-m-d\TH:i') : '';

        // FAKTA HYDRATION: Mengisi Kanvas dengan Gambar Lama dari DB
        foreach ($chapter->panels as $panel) {
            $this->panels[] = [
                'uuid' => uniqid('panel_'),
                'type' => 'existing',
                'id' => $panel->id,
                'url' => asset('storage/' . $panel->image_path),
                'name' => 'Panel Lama ' . ($panel->order_index + 1),
            ];
        }
    }
    ...
    {
        $this->validate(['temporary_uploads.*' => 'image|max:5120']);
    ...
    public function updateEpisode()
    {
        $this->validate([
            ...
            'new_ep_thumbnail' => 'nullable|image|max:1024',
            'panels' => 'required|array|min:1',
            'published_at' => 'nullable|date',
        ], ['panels.required' => 'Episode harus memiliki setidaknya 1 gambar.']);

        \DB::transaction(function () {
            // 1. Update Metadata
            if ($this->new_ep_thumbnail) {
                Storage::disk('public')->delete($this->chapter->thumbnail);
                $this->chapter->thumbnail = $this->new_ep_thumbnail->store('chapters/thumbnails', 'public');
            }
            $this->chapter->title = $this->ep_title;
            $this->chapter->creator_note = $this->creator_note;
            $this->chapter->allow_comments = $this->allow_comments;
            $this->chapter->published_at = $this->published_at ?: null;
            $this->chapter->status = 'pending_review';
            $this->chapter->save();

            // 2. Mesin Sinkronisasi Panel Terpadu
            $manager = new ImageManager(new Driver());
            $globalOrderIndex = 0;
            $keptExistingIds = [];

            // Kumpulkan ID dari panel lama yang tidak dihapus pengguna
            foreach ($this->panels as $panel) {
                if ($panel['type'] === 'existing') {
                    $keptExistingIds[] = $panel['id'];
                }
            }

            // Hapus panel di DB (dan storage fisik) yang sudah dibuang oleh pengguna di antarmuka
            $panelsToDelete = $this->chapter->panels()->whereNotIn('id', $keptExistingIds)->get();
            foreach ($panelsToDelete as $ptd) {
                Storage::disk('public')->delete($ptd->image_path);
                $ptd->delete();
            }

            // Susun Ulang & Potong Gambar Baru
            foreach ($this->panels as $panel) {
                if ($panel['type'] === 'existing') {
                    // Update index urutan jika bergeser
                    Panel::where('id', $panel['id'])->update(['order_index' => $globalOrderIndex++]);
                } elseif ($panel['type'] === 'new') {
                    // Potong dan insert data baru di urutan yang tepat
                    ...
                    if ($height > 1280) {
                        ...
                        for ($i = 0; $i < $slices; $i++) {
                            ...
                            Panel::create(['chapter_id' => $this->chapter->id, 'image_path' => $path, 'order_index' => $globalOrderIndex++]);
                        }
                    } else {
        ...
        session()->flash('success', 'Episode berhasil direvisi!');
        $this->redirectRoute('author.episodes.index', ['comic' => $this->comic->id], navigate: true);
    }
}; ?>

<div x-data="{ previewMode: false, device: 'mobile' }">
    <x-common.page-breadcrumb pageTitle="Revisi Episode {{ $chapter->chapter_number }}" />
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 lg:p-8">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-5 lg:mb-7 border-b border-gray-200 dark:border-gray-800 pb-4">
            <h3 class="text-xl font-bold text-gray-800 dark:text-white">Edit Episode: <span class="text-brand-500">{{ $comic->title }}</span></h3>
        ...
        <form wire:submit="updateEpisode">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="space-y-6">
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Thumbnail Baru (Opsional)</label>
                        <input wire:model="new_ep_thumbnail" type="file" accept="image/*" class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400" />
                        @if ($new_ep_thumbnail) <img src="{{ $new_ep_thumbnail->temporaryUrl() }}" class="mt-3 h-24 object-cover rounded shadow border-2 border-brand-500">
                        @else <img src="{{ asset('storage/' . $chapter->thumbnail) }}" class="mt-3 h-24 object-cover rounded shadow opacity-80"> @endif
                        <x-input-error :messages="$errors->get('new_ep_thumbnail')" class="mt-1 text-sm text-red-500" />
                    </div>
                ...
                <div class="lg:col-span-2">
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Susunan & Pembaruan Gambar</label>
                    <p class="text-xs text-gray-500 mb-4 dark:text-gray-400">Hapus gambar lama, unggah gambar baru, dan seret (*drag*) untuk mengatur urutan sesuka hati.</p>
                            ...
                            <ul class="space-y-2" x-data="{ draggingUuid: null }">
                                @foreach($panels as $index => $panel)
                                    <li data-uuid="{{ $panel['uuid'] }}" draggable="true"
                                        ...
                                        "
                                        class="flex items-center justify-between bg-white dark:bg-gray-900 p-3 rounded border {{ $panel['type'] === 'new' ? 'border-brand-300' : 'border-gray-200 dark:border-gray-700' }} cursor-move shadow-sm hover:border-brand-500 transition-colors">
                                        ...
                                        <div class="flex items-center space-x-4">
                                            ...
                                            <img src="{{ $panel['url'] }}" class="h-12 w-12 object-cover rounded shadow-sm border border-gray-100 dark:border-gray-800">
                                            <div class="flex flex-col">
                                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300 truncate w-32 md:w-56">{{ $panel['name'] }}</span>
                                                @if($panel['type'] === 'new')
                                                    <span class="text-xs text-brand-500 font-semibold">Sisipan Baru</span>
                                                @else
                    ...
                    @else
                        <div class="text-center py-8 text-gray-500 bg-gray-50 rounded-lg border border-dashed border-gray-300">Semua gambar telah dihapus. Harap unggah minimal 1 gambar.</div>
                    @endif
                </div>
            </div>
            ...
            <div class="mt-8 flex justify-end pt-5 border-t border-gray-200 dark:border-gray-800">
                <button type="submit" class="bg-brand-500 text-white px-8 py-3 rounded-lg font-medium hover:bg-brand-600 transition flex items-center shadow-theme-md">
                    <span wire:loading.remove wire:target="updateEpisode">Simpan Revisi Final</span>
                    <span wire:loading wire:target="updateEpisode">Memproses Data & Gambar...</span>
                </button>
            </div>
        </form>
    </div>
