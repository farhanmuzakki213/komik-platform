...
<div class="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-16">
    ...
    <section>
        ...
        <div
            class="w-full h-[100px] bg-[#1a1c3d] rounded-lg flex items-center justify-center cursor-pointer overflow-hidden relative">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 sm:gap-4">
            @foreach ($newReleases as $comic)
                <a href="{{ route('originals') }}"
                    class="group block relative aspect-[10/16] rounded-lg overflow-hidden bg-gray-100 shadow-sm border border-gray-200">
                    <img src="{{ asset('storage/' . $comic->vertical_thumbnail) }}"
                        class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                    <div
                    ...
                    <h3
                        class="absolute bottom-4 left-4 right-4 text-white font-black text-lg leading-tight text-center drop-shadow-lg">
                        {{ $comic->title }}</h3>
                </a>
            @endforeach
        </div>
    ...
    <section>
        ...
        <div class="flex justify-between items-end mb-4">
            <p class="text-[13px] text-gray-500"><span class="font-bold text-black">{{ count($dailyComics) }}</span>
                series</p>
                ...
                <a href="{{ route('comics.show', $comic->id) }}" class="group block">
                    <div class="relative aspect-[10/16] rounded-lg overflow-hidden mb-2 bg-gray-100">
                        <img src="{{ asset('storage/' . $comic->vertical_thumbnail) }}"
                            class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                        <div
                            class="absolute top-1.5 left-1.5 bg-[#00dc64] text-white text-[9px] font-bold px-1.5 py-0.5 rounded-sm uppercase tracking-wide">
                            New Episode
                        </div>
                    </div>
